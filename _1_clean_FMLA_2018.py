...
class DataCleanerFMLA:
    ...
    def impute_fmla_cps(self, fp_cps_in, fp_cps_out):
        ...
        cps = pd.read_stata(fp_cps_in, convert_categoricals=False)
        ...
        # union
        # yvar - a_unmem (defined only for prerelg=1, which marks non-self-employed private sector worker)
        # see https://data.nber.org/morg/docs/cpsx.pdf
        # so prerelg=0 must have union=0
        cps['union'] = np.nan
        cps['union'] = np.where(cps['a_unmem'] == 1, 1, cps['union'])
        cps['union'] = np.where(cps['a_unmem'] == 2, 0, cps['union'])
        cps['union'] = np.where(cps['prerelg'] != 1, 0, cps['union'])
        # union=NA if prerelg=1 and a_unmem=0
        ...
        cps = cps[['peridnum', 'marsupwt'] + # unique person id, weight
                  ...
                  ['hourly', 'empsize', 'oneemp', 'union', 'fmla_eligible']]  # remove armed forces code from ind/occ

        ## FMLA 2018: no longer need code below to sim FMLA vars
