...
methods <- c('Exhibit','Measure','Type','Actual','random','logit','random_forest','ridge_class','xg_boost','KNN1')
...
ggplot(data=prop_df %>%  mutate(Method = factor(Method, levels=c("random", "KNN1", "logit", "xg_boost", "random_forest", "ridge_class")))
      ...
      xlab(NULL) + 
      scale_x_discrete(labels = c("Random", "KNN1", "Logit", "xGBoost", "Random\nForest", "Ridge Class")) +
...
ggsave(file="Exhibits/IB2_1_prop_pay_avg.png", width=6.5, dpi=300)
...
ggplot(data=prop_df %>% filter(Method != 'random'), aes(x=Method, y=Accuracy)) +
  ...
  xlab(NULL) + 
  scale_x_discrete(labels = c("KNN1", "Logit", "xGBoost", "Random\nForest", "Ridge Class")) +
  theme(axis.text.x = element_text(size = 10)) +
...
ggsave(file="Exhibits/IB2_2_prop_pay_acc.png", width=6.5, dpi=300)
...
# ggsave(file="Exhibits/IB2_3_pred_type_acc.png", width=6.5, dpi=300)
# 
# # add results to heat map df
...
ggplot(data=num_df %>%  mutate(Method = factor(Method, levels=c("random", "KNN1", "logit", "xg_boost", "random_forest", "ridge_class"))), aes(x=Method, y=Predicted_Leave_Num)) +
...
ggsave(file="Exhibits/IB2_4_num_leaves_tot.png", width=6.5, dpi=300)
...
# ggsave(file="Exhibits/IB2_5_num_leaves_acc.png", width=6.5, dpi=300)
# 
# # add results to heat map df
...
bi_df_filt <- bi_df %>% filter(Variable == 'pred_leave') %>%  mutate(Method = factor(Method, levels=c("random", "KNN1", "logit", "xg_boost", "random_forest", "ridge_class")))
...
ggsave(file="Exhibits/IB2_6_leave_takers_tot.png", width=6.5, dpi=300)
...
ggplot(data=bi_df_filt %>%  mutate(Method = factor(Method, levels=c("random", "KNN1", "logit", "xg_boost", "random_forest", "ridge_class"))), aes(x=Method, y=Predicted_Positives)) +
...
ggsave(file="Exhibits/IB2_7_leave_takers_acc.png", width=6.5, dpi=300)
...
# ggsave(file="Exhibits/IB2_8_leave_takers_precision.png", width=6.5, dpi=300)
# 
# # --------------------------------------------------------------------------------------------------------------------
...
# ggsave(file="Exhibits/IB2_9_leave_takers_recall.png", width=6.5, dpi=300)
...
methods <- c("random", "KNN1", "logit", "xg_boost", "random_forest", "ridge_class")
for (meth in methods) {
  df <- read.csv(paste0('./output/RI_',meth,"issue_brief_2_rawstats.csv"))
...
ggplot(data=ri_df  %>%  mutate(Method = factor(Method, levels=c("random", "KNN1", "logit", "xg_boost", "random_forest", "ridge_class")))
...
methods <- c("random", "KNN1", "logit", "xg_boost", "random_forest", "ridge_class")
for (meth in methods) {
  df <- read.csv(paste0('./output/NJ_',meth,"issue_brief_2_rawstats.csv"))
  result <- df %>% filter(Variable == 'Benefits Received ($), total')
  row <- matrix(c(meth, result[[5]],result[[6]],result[[5]]- 1.96*result[[6]],result[[5]] + 1.96*result[[6]]), ncol = ncols, nrow = 1)
...
ggplot(data=nj_df  %>%  mutate(Method = factor(Method, levels=c("random", "KNN1", "logit", "xg_boost", "random_forest", "ridge_class")))
       , aes(x=Method, y=Estimate)) +
  # bar chart
  geom_bar(stat="identity", fill = '#D6CFAC') +
  theme_minimal() +
...
methods <- c("random", "KNN1", "logit", "xg_boost", "random_forest", "ridge_class")
for (meth in methods) {
  df <- read.csv(paste0('./output/CA_',meth,"issue_brief_2_rawstats.csv"))
  result <- df %>% filter(Variable == 'Benefits Received ($), total')
  row <- matrix(c(meth, result[[5]],result[[6]],result[[5]]- 1.96*result[[6]],result[[5]] + 1.96*result[[6]]), ncol = ncols, nrow = 1)
...
ggplot(data=ca_df  %>%  mutate(Method = factor(Method, levels=c("random", "KNN1", "logit", "xg_boost", "random_forest", "ridge_class")))
       , aes(x=Method, y=Estimate)) +
  # bar chart
  geom_bar(stat="identity", fill = '#D6CFAC') +
  theme_minimal() +
...
# --------------------------------------------------------------------------------------------------------------------
# reorder columns
heat_df <- heat_df %>% select(Exhibit, Method, Type, Actual, random, logit, KNN1,random_forest, xg_boost, ridge_class)
heat_df <- heat_df[order(heat_df$Exhibit),] 
  
# export to xlsx for touching up in excel
write.csv(heat_df, file="./Exhibits/IB2_heat_map_raw.csv", row.names=FALSE)